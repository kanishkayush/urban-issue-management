...
export function IssueProvider({ children }) {
  const [issues, setIssues] = useState(initialMockIssues);
  const [loading, setLoading] = useState(false);

  useEffect(() => {
    // Only attempt to fetch if the client was successfully configured
    if (supabase) {
  ...
  const fetchIssuesFromSupabase = async () => {
    ...
    if (error) {
      ...
      // Map Supabase column names to our local app structure
      const mappedData = data.map(row => ({
        id: row.id.substring(0, 8), // Shorten UUID for UI
        type: row.type,
        location: row.location || 'Jaipur (GPS Sourced)',
        description: row.description,
        priorityScore: row.priorityScore || 0,
        keywords: row.aiKeywords || [],
        image: row.image,
        date: new Date(row.date).toLocaleString(),
        // Assigning dummy coordinates for the map since we aren't storing lat/lng yet
        coordinates: [26.9150 + (Math.random() - 0.5) * 0.05, 75.7900 + (Math.random() - 0.5) * 0.05] 
      }));
  ...
  const addIssue = async (newIssue) => {
    ...
    if (supabase) {
      // Insert to Supabase DB if configured
      const { data, error } = await supabase
        .from('reports')
        .insert([{
          type: fullIssueLocal.type,
          description: fullIssueLocal.description,
          location: fullIssueLocal.location,
          "priorityScore": fullIssueLocal.priorityScore,
          image: fullIssueLocal.image,
          "aiKeywords": fullIssueLocal.keywords
        }])
        .select();

      if (error) {
        console.error('Error inserting to Supabase:', error);
        alert('Error saving issue to database.');
      } else if (data) {
        // Fetch again to sync the exact structure
        fetchIssuesFromSupabase();
      }
    } else {
      // Fallback: Use Local State memory array
      fullIssueLocal.id = `ISS-${1050 + issues.length + 1}`;
      fullIssueLocal.date = 'Just now';
      fullIssueLocal.coordinates = [26.9124 + (Math.random() - 0.5) * 0.05, 75.7873 + (Math.random() - 0.5) * 0.05];
      setIssues(prev => [fullIssueLocal, ...prev]);
    }
  };

  return (
    <IssueContext.Provider value={{ issues, loading, addIssue }}>
      {children}
